...
transformed parameters {
  ...
  weight_sim[ii_obs] = 1/(p1[ii_obs] + 2*p2[ii_obs] + 3*p3[ii_obs] + 4*p4[ii_obs] + 5*p5[ii_obs] + 6*p6[ii_obs] + 7*p7[ii_obs] + 8*p8[ii_obs] + 9*p9[ii_obs] + 10*p10[ii_obs]);
  
  for (j in 1:J_mis) {
    //weight_sim[j] = 1/(1 - p1[j] - pb_NB[j]);
    weight_sim[j] = 1/(p1[j] + 2*p2[j] + 3*p3[j] + 4*p4[j] + 5*p5[j] + 6*p6[j] + 7*p7[j] + 8*p8[j] + 9*p9[j] + 10*p10[j]);
  }
