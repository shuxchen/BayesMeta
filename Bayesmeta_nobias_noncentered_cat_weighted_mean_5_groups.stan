...
transformed parameters {
  ...
  vector<lower=0, upper=1>[J] p5;     // % with fifth entrant
  ...
  vector<lower=0, upper=1>[J_mis] p5_NB;     // % with fifth entrant
  ...
  p6_sum[ii_obs] = 1 - p1[ii_obs] - p2[ii_obs] - p3[ii_obs] - p4[ii_obs] - p5[ii_obs];

  
  for (j in 1:J_mis) {
    weight_sim[j] = 1/(1 - p1[j] - pb_NB[j]);
    p6_sum[j] = 1 - p1[j] - p2[j] - p3[j] - p4[j] - p5[j];
  }
  
  for (j in 1:J) {
    gamma[j] = mu1 * p2[j]' * weight_sim[j] + mu2 * p3[j]' * weight_sim[j] + mu3 * p6_sum[j]' * weight_sim[j] + tau1 * gamma1_tilde[j] + tau2 * gamma2_tilde[j] + tau3 * gamma3_tilde[j];
