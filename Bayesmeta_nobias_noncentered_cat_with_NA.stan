data {
  ...
  int<lower = 1, upper = J_obs + J_mis> ii_obs[J_obs];  // number of studies with pi given 
  int<lower = 1, upper = J_obs + J_mis> ii_mis[J_mis];  // number of studies with pi given 
...
transformed parameters {
  ...
  for (j in ii_mis) {
    N4_mis[j] = (N_mean[j] - 1 - p1[j] - p2[j])/p3[j];
  }
