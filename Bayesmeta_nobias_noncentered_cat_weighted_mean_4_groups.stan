data {
  ...
  vector[P-5] beta1;                         // estimated beta coefficient, for k = 5, ..., 10
  vector[P-5] beta2;                         // estimated beta coefficient, for k = 5, ..., 10
  vector[P-5] beta3;                         // estimated beta coefficient, for k = 5, ..., 10
  vector[P-5] beta4;                         // estimated beta coefficient, for k = 5, ..., 10

  vector<lower=0>[P-5] sigma1;                     // standard error of beta1 coefficient
  vector<lower=0>[P-5] sigma2;                     // standard error of beta2 coefficient
  vector<lower=0>[P-5] sigma3;                // standard error of beta3 coefficient
  vector<lower=0>[P-5] sigma4;                // standard error of beta4 coefficient, for k = 5, ..., 10
...
parameters {
  ...
  vector[P-5] gamma1_cat_tilde;             // per study effect
  vector[P-5] gamma2_cat_tilde;             // per study effect
  vector[P-5] gamma3_cat_tilde;             // per study effect
  vector[P-5] gamma4_cat_tilde;             // per study effect

  real mu1;                               // mean effect of second entrant
  real mu2;                               // mean effect of third entrant
  real mu3;                               // mean effect of 4th entrants
  real mu4;                               // mean effect of 5+ entrants
...
transformed parameters {
  ...
  vector<lower=0, upper=1>[J] p4;     // % with fourth entrant
  ...
  vector<lower=0, upper=1>[J_mis] p4_NB;     // % with fourth entrant
  
  vector<lower=0>[J] weight_sim;    
  vector<lower=0>[J] p5_sum;     
  ...
  p5_sum[ii_obs] = 1 - p1[ii_obs] - p2[ii_obs] - p3[ii_obs] - p4[ii_obs];

  
  for (j in 1:J_mis) {
    weight_sim[j] = 1/(1 - p1[j] - pb_NB[j]);
    p5_sum[j] = 1 - p1[j] - p2[j] - p3[j] - p4[j];
  }
  
  for (j in 1:J) {
    gamma[j] = mu1 * p2[j]' * weight_sim[j] + mu2 * p3[j]' * weight_sim[j] + mu3 * p5_sum[j]' * weight_sim[j] + tau1 * gamma1_tilde[j] + tau2 * gamma2_tilde[j] + tau3 * gamma3_tilde[j];
